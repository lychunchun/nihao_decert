...
contract NFTMarket is ITokenReceiver {
    ...
    function list(address _nftContract, uint256 _tokenId, uint256 _price) external returns (uint256) {
        ...
        // 触发NFT上架事件
        emit NFTListed(listingId, owner, _nftContract, _tokenId, _price); // 添加此行
    ...
    function buyNFT(uint256 _listingId) external {
        ...
        // 触发NFT售出事件
        emit NFTSold(_listingId, msg.sender, listing.seller, listing.nftContract, listing.tokenId, listing.price); // 添加此行
    }
    ...
    function tokensReceived(address from, uint256 amount, bytes calldata data) external override returns (bool) {
        ...
        // 触发NFT售出事件
        emit NFTSold(listingId, from, listing.seller, listing.nftContract, listing.tokenId, amount); // 添加此行
